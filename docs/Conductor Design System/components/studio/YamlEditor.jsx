          ...
          >
            ...
            <span style={{ whiteSpace: "pre-wrap", wordBreak: "break-word", minWidth: 0 }}>
              {tokenize(text).map((tk, j) => (
                <span key={j} style={{ color: COLORS[tk.t] }}>{tk.v}</span>
              ))}
              {isActive ? (
                <span style={{ display: "inline-block", width: 1.5, height: "1.05em", marginLeft: 1, verticalAlign: "text-bottom", background: "var(--editor-caret)", animation: "cd-caret 1s steps(1) infinite" }} />
              ) : null}
            </span>
          </div>
        );
