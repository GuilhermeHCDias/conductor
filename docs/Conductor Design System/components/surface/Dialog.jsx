...
export function Dialog({ open = true, title, subtitle, icon, children, footer, width = 460, onClose, style, ...rest }) {
  if (!open) return null;
  return (
    <div
      style={{
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", gap: "var(--space-5)", padding: "var(--space-6) var(--space-6) var(--space-5)" }}>
          {icon ? (
            <span style={{ display: "grid", placeItems: "center", width: 32, height: 32, flex: "none", borderRadius: "var(--radius-sm)", background: "var(--accent-quiet)", color: "var(--accent)" }}>
              <Icon name={icon} size={16} />
            </span>
          ) : null}
          <div style={{ flex: 1, display: "grid", gap: 3, minWidth: 0 }}>
            <h2 style={{ font: "var(--type-title-3)", color: "var(--text-primary)" }}>{title}</h2>
            {subtitle ? <p style={{ font: "var(--type-body)", color: "var(--text-secondary)" }}>{subtitle}</p> : null}
          </div>
        ...
        {footer ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "var(--space-4)", padding: "var(--space-5) var(--space-6)", borderTop: "var(--border-hair) solid var(--edge-1)", background: "var(--glass-sunken)" }}>
            {footer}
          </div>
        ) : null}
      </div>
