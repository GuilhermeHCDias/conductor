...
- **Post-review adjustments (product owner, 2026-08-08):** (1) The describe invocation runs on the fastest alias — `--model haiku`, `CONFIG.AI_DESCRIBE_MODEL` — overriding criterion 10's literal `--model sonnet`; §6.0's sonnet stays the AIPanel's. (2) The sheet's dismiss button reads "Cancel", not the kit's "Not yet". (3) The Dialog fill is near-opaque (`--glass-dialog`, both themes): the kit's `--glass-3` frost let the app behind the modal fight the sheet's own text. (4) A repo without `conductor/` gets the folder seeded with `.gitkeep` by the flow workspace — an empty folder never ships in Git — so the folder itself rides the next publication as an ordinary criterion-8 change.
- **The note is required (product owner, 2026-08-08) — amends criteria 13 and 17, and `.context.md` §8.4/§8.5/rule 24 again:** "the person always has to describe what was done". Sending needs a non-blank note, and waits for the describe job to finish — a sentence still being written is one nobody has read. The rule lives in the store (`selectCanSend`, plus the guard in `sendForReview`), so the disabled button only announces it; the change set's own emptiness stays main's call (criterion 24). The field's hint reads "Required — your team reads this." in place of the optional one. In practice the field is never empty when it lands — the mechanical fallback of criterion 15 guarantees it — so the person confirms or adjusts rather than composes.
- **The field is locked while the AI writes (product owner, 2026-08-08) — amends criterion 13:** the "unless the person already typed, in which case their text is never overwritten" branch is gone, because there is no longer a way to type first. `readOnly` rather than `disabled` — the field is being written into, not unavailable, so it keeps its tab-order place, stays announced beside `aria-busy`, and keeps its own colours under the writing treatment; the caret and the text cursor go, so it never invites a sentence it would refuse. `disabled` still stands for the sending phase, where the field really is unavailable. The rule lives in the store (`editNote` refuses while `writing`) the way criterion 17's does, so the attribute only announces it. Consequence: the generated note now always lands, so "arrives prefilled, the person confirms or adjusts" has no exception left.
- **The describe is one turn with no tools (product owner, 2026-08-08) — amends criterion 10 and `.context.md` §8.4/rule 22 again:** `haiku` alone did not fix the wait. Measured here, the criterion-10 profile took **14.1 s** to write two lines: ~1137 output tokens for a 60-token answer (extended thinking, useless for a fixed-format writing task) across **5 turns** — load the skill, read `changed-files.txt`, read `diff.patch`, answer. The replacement measures **1.5 s** at ~$0.0024 against $0.026, with text of equal quality (verified live, repeated, including over an 8-file diff). Concretely: the listing and the patch are inlined in the prompt instead of `--add-dir`'d as a job dir; the skill's own `SKILL.md` body is read by us and passed as `--system-prompt` instead of loaded by the model; `--tools ""` replaces `Skill,Read,Glob,Grep`; `--plugin-dir` drops (no `Skill` tool, nothing to load); `--effort low` and `MAX_THINKING_TOKENS=0` in the child's env kill the thinking. §8.4's isolation only tightens — a model with no tools cannot read, write or run anything, and criterion 11 becomes literal. Two costs, both paid: the publication diff now carries `-U25` so a changed flow still arrives essentially whole without `Read`, and the patch is truncated (and says so) past 120 000 characters because argv is OS-bounded — the file listing sits above the cut, so a too-large publication never loses the list of what changed. The skill file stays exactly where it was and stays the one place a bad note is fixed; the plugin stays what §6.0 loads for the AIPanel.
- **The writing state is drawn (same decision):** while the describe job runs, the field carries the AI generation treatment — the aurora triad travelling around its edge (a conic gradient over a registered `--note-sweep` angle, masked to the 1px edge), the halo it casts, and a slow sheen crossing the surface — with the label showing a breathing sparkle beside "Writing…". It is the app's first *state* animation, so `styles.test.ts`'s motion guard gains a fourth allowed shape; every clock is a multiple of `--dur-lazy`, which reduced motion zeroes at the token.
